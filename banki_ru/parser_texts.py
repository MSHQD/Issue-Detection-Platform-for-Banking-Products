...
def main() -> None:
    ...
    if not isinstance(links, list) or not links:
        print("Файл links.json пустой или не список")
        return
    ...
    with sync_playwright() as p:
            ...
            try:
                ...
                if json_ld_text:
                    try:
                        ...
                        print(f"JSON-LD parse error on id={rid}: {e}")
                ...
                if date_iso and not in_range(date_iso):
                    print(f"Пропущен вне диапазона: {date_iso}")
                    skipped_by_date += 1
                    continue
                ...
                try:
                    ...
                    print(f"Ошибка при закрытии вкладки (id={rid}): {e}")
                delay_ms(1000)
        ...
        print(f"Пропущено по датам: {skipped_by_date}")
